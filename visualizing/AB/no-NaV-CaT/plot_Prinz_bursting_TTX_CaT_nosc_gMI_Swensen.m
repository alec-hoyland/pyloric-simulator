...
% Swensen modulatory input conductance (Swensen \& Marder 2001).
...
suptitle('all Prinz bursting models w/o NaV or CaT w/ Swensen MI')
prettyFig()
...
suptitle('excised Prinz bursting models w/o NaV or CaT w/ Swensen MI')
prettyFig()
...
%% Make Histograms for Thesis Talk
c = linspecer(2);
% burst frequency at gMI = [0, 1]
burst_freq_hist = burst_freq([1, 11], :);
amplitude_hist = amplitude([1, 11], :);

figure('outerposition',[2 2 1500 1000],'PaperUnits','points','PaperSize',[1500 1000]);
ax(1) = subplot(1,2,1); hold on
histogram(burst_freq_hist(1,:),'BinEdges',linspace(0,max(vectorise(burst_freq_hist)),11), 'DisplayStyle', 'stairs', 'LineWidth', 6, 'EdgeColor', c(1,:));
histogram(burst_freq_hist(2,:),'BinEdges',linspace(0,max(vectorise(burst_freq_hist)),11), 'DisplayStyle', 'stairs', 'LineWidth', 6, 'EdgeColor', c(2,:));
legend('decentralized','modulated')
ylabel('# of models')
xlabel('frequency (Hz)')
prettyFig('fs',36)

ax(2) = subplot(1,2,2); hold on
histogram(amplitude_hist(1,:),'BinEdges',linspace(0,max(vectorise(amplitude_hist)),11), 'DisplayStyle', 'stairs', 'LineWidth', 6, 'EdgeColor', c(1,:));
histogram(amplitude_hist(2,:),'BinEdges',linspace(0,max(vectorise(amplitude_hist)),11), 'DisplayStyle', 'stairs', 'LineWidth', 6, 'EdgeColor', c(2,:));
