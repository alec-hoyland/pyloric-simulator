...
% load data
if ~exist('~/code/pyloric-simulator/data/thesis/data_plot_network_AB.mat')
	allfiles = dir('data_optim_network_AB*.mat');
	...
	% save data
	save('~/code/pyloric-simulator/data/thesis/data_plot_network_AB.mat', ...
		'V', 'burst_freq', 'duty_cycle', 'nSpikes', 'min_slow_wave', 'max_slow_wave', ...
		'params','parameter_names','nModels');
else
	load('~/code/pyloric-simulator/data/thesis/data_plot_network_AB.mat')
end

% get gMI values
for ii = 1:nModels
	gMItrix(:,ii) = linspace(0,params(find(contains(parameter_names,'AB.MICurrent.gbar')),ii),nSims);
end
% get amplitude
