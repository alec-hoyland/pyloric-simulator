...
function [C, cost_vector, metrics_vector, V, Ca] = isPyloric(x,posthoc)
...
% slow wave minima (19-21)
% slow wave maxima (22-24)
...
burst_duration_range(:,1) = [0.4490/4  0.7150*2]; % PD, sec
burst_duration_range(:,2) = [0.2860/4 .5120*2]; % LP, sec
burst_duration_range(:,3) = [0.38/4 .68*2]; % PY, sec
...
Gap_PD_LP_range = [.112/4 .33*2];
Gap_LP_PY_range = [-.121/4 -.001*2];

% delays are X_start -> B_start
Delay_PD_LP_range = [.6340/4 .9720*2]; % seconds
Delay_PD_PY_range = [.9250/4 1.3570*2]; % seconds
...
C = sum(cost_vector(:));
if any(cost_vector(1,:) > 1) & ~posthoc
	return
end
...
% early exit
if any(cost_vector(2,:) > 1) & ~posthoc
	C = sum(cost_vector(:));
	return
end
...
for i = 1:3
	...
	if length(ons) ~= length(offs) & ~posthoc
		return
	end
...
if any(cost_vector(3,:) > 1) & ~posthoc
	return
end
...
sync_cost = level_cost*bin_cost([.95 1.05],max(all_periods)/min(all_periods));
...
% measure slow wave
for ii = 1:3
	[metrics_vector(18+ii), metrics_vector(21+ii)] = findSlowWaveStats(x.dt,V(:,ii));
end
